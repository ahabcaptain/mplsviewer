...
foreach ($vrfs as $entry)
{
	echo "<OPTION VALUE=".$entry['name']." ";
	if ($_POST["select"]==$entry['name']) {echo "selected";} 
...
{
	$data = get_data($table);
	$vrfdata = get_data_vrf($table, $_POST["select"]);
...
//	echo "<th BGCOLOR='#A0A0A0'>Filters</th>";
	echo "</tr>";
	foreach (super_unique($vrfdata,'device') as $entry)
	{
		echo "<tr>";
		echo "<td BGCOLOR='#808080' style='width: 95px;'><center><b>";
                echo "<a style='text-decoration:none' href='show-one-device.php?select=".$entry['device'];
...
// Import Communities start
		...
		{
			if($subentry['device']==$entry['device'] && $subentry['import']!='')
			{
...
// Export communities start
		...
		echo "<td style='vertical-align:top;'><center>";
                ...
                foreach ($vrfdata as $subentry)
                {
                        if($subentry['device']==$entry['device'] && $subentry['export']!='')
                        {
                                echo "<tr><td style='padding-top: 5px; vertical-align:top; background-color: #F0F0F0; width: 100px; border-bottom: solid black 1px'><center><b>";
